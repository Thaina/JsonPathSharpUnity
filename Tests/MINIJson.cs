...
namespace JsonPath.Tests
{
	...
	public class MiniJSON
	{
		...
		private static System.Object lockDecode = new System.Object();
		/// <summary>
		...
		public static char[] buffer = new char[0];
				...
				else if(c == '\\')
				{
					...
					else if(c == 'u')
					{
						...
						if(remainingLength >= 4)
						{
							...
							//sb.Append ("&#x").Append (new string (unicodeCharArray)).Append (";");

							uint codePoint = UInt32.Parse(new string(unicodeCharArray),NumberStyles.HexNumber);
							// convert the integer codepoint to a unicode char and add to string
							//sb.Append (Char.ConvertFromUtf32 ((int)codePoint));

							buffer[bufferIndex] = Char.ConvertFromUtf32((int)codePoint)[0];
							bufferIndex++;
		...
		protected static double parseNumber(char[] json,ref int index)
		{
			...
			Array.Copy(json,index,buffer,0,charLength);
			index = lastIndex + 1;
			//return Double.Parse (new string (numberCharArray)); // , CultureInfo.InvariantCulture);
			return double.Parse(new string(buffer,0,charLength));
		}
		...
		protected static bool serializeDictionaryObj(Dictionary<string,object> dict,StringBuilder builder)
		{
			builder.Append("{");
		...
		protected static bool serializeValue(object value,StringBuilder builder)
		{
			...
			{
				serializeDictionaryObj((Dictionary<string,object>)value,builder);
			}
